...
exports.global = function( names ) {
	...
	return exports.atExpansion( function() {
		var env = this.env;
		names.replace( /\w+/g, function( name ) {
			if( !( name in env ) )
				env[name] = undefined;
		} );
		return "";
...
exports.promise = function( name, value ) {

	var promises;

	if( typeof name !== 'string' || !value || !value.expr )
		throw Error( "The only supported syntax is  @ promise: name := expression @" );

	(promises = this.body.promises || (this.body.promises = []))
		.push( { name: name, value: value.expr } );
	
	return function() {
		if( !promises.errMetadata )
			promises.errMetadata = this._tryStack && this._tryStack[this._tryStack.length-1] || [];
		return "";
	}
}
