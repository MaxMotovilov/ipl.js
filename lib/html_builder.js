...
exports.rawInput = function( text ) {
	...
	return build.atExpansion( function() {
		return this._iplWrapped ? build.makeOutput( build.stringLiteral( text ) ) : text;
	} );
}

exports.substitution = function( text, raw, at_build_time ) {
	return [
		!raw ? build.once( enc$html ) : "",
		at_build_time ? build.once( to$string ) : "",
		!raw && !at_build_time ? ipl.once( enc$html.toString() ) : "",
		makeOutput( raw ? text : "enc$html(" + text + ")", at_build_time )
	];
}
