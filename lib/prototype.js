...
function finalRender( result ) {
	return promise.when(
		resolveCompletely( result, identity, flattenStrings ),
...
function flattenStrings( output, segment ) {
	var first = segment, rest;

	if( segment instanceof Array ) {
		rest = segment.reduce( flattenStrings, [] );
		first = rest.shift();
	}
	
	if( typeof first === "string" && typeof output[output.length-1] === "string" )
