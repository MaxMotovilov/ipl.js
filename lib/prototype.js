...
exports.precompile = function( input, is_script ) {
	...
	return promise.when( load(), parse ).then( identity, fail );
	...
	function load() {
		if( input instanceof stream.Readable )
			...
			tagname = (is_script ? "ipl:" : "html:") + input;
	...
	function build() {
		...
		function result( args ) {
			...
			var _this = this;
			return promise.allOrNone( body.map( function( segment ) {
				if( typeof segment === 'function' || typeof segment === 'string' )
					return segment;
				var result = evalInEnv(
					'function() {\n' +
						'return 
				
				

			var expanded = body.reduce( function( acc, segment ) {
			...
			return evalInEnv( 
					'function() {\n' +
						body.map( function( segment ) {
							return typeof segment === 'string' ? segment : segment.apply( _this );
						} ).join( '' ) +
					+ '\n}',
					_this.env
			)( args );
...
exports.loadFragment = function( name, is_script, args ) {
	var cache = _this.cache[ is_script ? "ipl" : "html" ];
	...
	if( name in this.builders ) {
		if( args.length ) {
			if( 'fragmentIsScript' in this.builders[name] )
				this.namedFragment( args.shift(), this.builders[name].fragmentIsScript, args );
			else
		...
		this.namedFragment( name, stack[0].fragmentIsScript, args );
	}
...
exports.namedFragment = function( name, is_script, args ) {
